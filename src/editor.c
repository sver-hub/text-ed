...
void editorSave()
{
  ...
  int fd = open(E.filename, O_RDWR | O_CREAT, 0644);  //0644 - permission 
  if (fd != -1)
  {
...
void editorProcessKeypress()
{
  ...
  switch (c)
  {
    ...
    case CTRL_KEY('q'):
      ...
      write(STDOUT_FILENO, "\x1b[H", 3);	//move to 1,1
      exit(0);
      break;
...
void editorDrawRows(struct abuf *ab)
{
  ...
  for (y = 0; y < E.screenrows; y++)
  {
    ...
    abAppend(ab, "\x1b[K", 3); //flush right to line
    abAppend(ab, "\r\n", 2);
  }
}
...
void editorDrawStatusBar(struct abuf *ab)
{
  //abAppend(ab, "\x1b[7m", 4); //inver colors
  ...
  //abAppend(ab, "\x1b[m", 3);  //invert back
  abAppend(ab, "\r\n", 2);
}
...
void editorRefreshScreen()
{
  ...
  snprintf(buf, sizeof(buf), "\x1b[%d;%dH", E.cy - E.rowoff + 1, 
                                            E.rx - E.coloff + 1);
  abAppend(&ab, buf, strlen(buf)); //move to cx,cy
